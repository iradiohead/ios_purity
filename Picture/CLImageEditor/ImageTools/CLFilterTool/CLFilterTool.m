...
+ (BOOL)isAvailable
{
    return TRUE;// ([UIDevice iosVersion] >= 5.0);
}
...
- (void)setup
{
    ...
    if(_originalImage)
    {
        ...
        _filterScroll = [[UIScrollView alloc] initWithFrame:self.editor.myScrollView.frame]; //self.editor.menuView.frame
        _filterScroll.backgroundColor = self.editor.myScrollView.backgroundColor;
        ...
        //jk: 先把scrollview向下移动，刚刚移到屏幕看不到的位置
        [UIView animateWithDuration:kCLImageToolAnimationDuration
			animations:^
			{
			 _filterScroll.transform = CGAffineTransformIdentity;//指没有设置transform之前坐标
			 //scrollview移到self.editor.myScrollView.frame位置
			}];
    }
}
...
{
    completionBlock(self.editor.myImageView.image, nil, nil);
}
...
- (void)setFilterMenu
{
    ...
    {
        if(1)//([UIDevice iosVersion] >= [filter[@"version"] floatValue])
        {

...
@implementation CLBlurTool
{
    ...
    UISlider *_brightnessSlider; //add by kjin
...
+ (BOOL)isAvailable
{
    return YES;//return ([UIDevice iosVersion] >= 5.0);
}

- (void)setup
{
    _blurType = kCLBlurTypeNormal;
    _originalImage = self.editor.myImageView.image;
    NSLog(@"@ORI WITDH = %f, height = %f",_originalImage.size.width,_originalImage.size.height);
    
    //if _originalimage = (640*480),USE UIViewContentModeScaleAspectFill will be show as (640*640/480)*640
    //_thumnailImage for show in imageview, when finish, _originalImage do the algorithm.
    // this app because we crop the image, so ori already is the thumnail size
    _thumnailImage =_originalImage;// [_originalImage resize:self.editor.myImageView.frame.size];
    
    _blurSlider = [self sliderWithValue:0.0 minimumValue:0 maximumValue:1 action:@selector(sliderDidChange:)];
    _blurSlider.superview.center = CGPointMake(self.editor.view.width/2, self.editor.myScrollView.center.y-20);
    
    
    _brightnessSlider = [self sliderWithValue:-0.2 minimumValue:-1 maximumValue:0 action:@selector(sliderDidChange:)];
    _brightnessSlider.superview.center = CGPointMake(self.editor.view.width/2, self.editor.myScrollView.center.y+20);

    
    //_handlerView is when click Circle button, show in imageview
    /*_handlerView = [[UIView alloc] initWithFrame:self.editor.myImageView.frame];
    [self.editor.myImageView addSubview:_handlerView];
    [self setHandlerView];*/
    
    /*
    int yPostion = self.editor.myScrollView.bounds.size.height+self.editor.myImageView.bounds.size.height;
    CGRect rectFrame = CGRectMake(0, yPostion, ScreenWidth, 50 );
    _menuScroll = [[UIScrollView alloc] initWithFrame:rectFrame];
    _menuScroll.backgroundColor = self.editor.myScrollView.backgroundColor;
...
- (void)executeWithCompletionBlock:(void(^)(UIImage *image, NSError *error, NSDictionary *userInfo))completionBlock
{
    ...
    ^{
        UIImage *blurImage = [_originalImage gaussBlur:_blurSlider.value brightness:_brightnessSlider.value];
        blurImage = [self filteredImage:_blurImage];
...
- (void)setDefaultParams
{
    ...
    CGFloat ratio = _originalImage.size.width / self.editor.myImageView.width;
...
{
    UISlider *slider = [[UISlider alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth-40, 30)];
    
    UIView * container = [[UIView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth-40, slider.height)];
...
- (void)sliderDidChange:(UISlider*)slider
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0),
    ^{
        _blurImage = [_thumnailImage gaussBlur:_blurSlider.value brightness:_brightnessSlider.value];
        _blurImage = [self filteredImage:_blurImage];
...
- (void)buildThumnailImage
{
    ...
    ^{
        ...
        [self.editor.myImageView performSelectorOnMainThread:@selector(setImage:) withObject:image waitUntilDone:NO];
        inProgress = NO;
    });
}
...
{
    CGFloat ratio = image.size.width / self.editor.myImageView.width;
